...
export function skeletonTable(rows = 5, cols = 4) {
    ...
    <div class="overflow-table">
      <table class="dota-table">
        <thead><tr>${Array(cols)
			.fill("")
			.map(() => '<th><div class="skeleton h-3 w-16"></div></th>')
			.join("")}</tr></thead>
        <tbody>
          ${Array(rows)
				.fill("")
            ...
            <tr>${Array(cols)
				.fill("")
				.map(() => '<td><div class="skeleton h-3 w-20"></div></td>')
				.join("")}</tr>
          `,
				)
				.join("")}
...
export function miniHeroImg(heroId, size = 24) {
	...
	if (!hero)
		return `<div class="inline-block bg-dota-card rounded" style="width:${size}px;height:${size * 0.56}px" title="Hero #${heroId}"></div>`;
