...
export function heroImg(hero) {
	if (!hero) return "";
	const name = (hero.name || hero.localized_name || "").replace(
		"npc_dota_hero_",
		"",
	);
...
export function formatMatchAvgSkill(match) {
	...
	if (computedMMRs.length > 0) {
		const averageComputedMMR = Math.round(
			computedMMRs.reduce((total, value) => total + value, 0) /
				computedMMRs.length,
		);
	...
	if (rankTiers.length > 0) {
		const averageTier = Math.round(
			rankTiers.reduce((total, value) => total + value, 0) /
				rankTiers.length,
		);
...
export const heroStore = {
	...
	getItemKey(key) {
		...
		if (Number.isFinite(numericId)) {
			const found = Object.entries(this.items).find(
