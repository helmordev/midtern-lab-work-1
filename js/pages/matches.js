...
export async function renderMatches() {
  ...
  `;
	...
	// Match lookup
	document
		.getElementById("match-lookup-btn")
	...
	// Parse request
	document
		.getElementById("parse-match-btn")
		.addEventListener("click", async () => {
			...
			try {
				...
				showToast(
					`Parse requested! Job: ${result.job?.jobId || "queued"}`,
					"success",
				);
			} catch {
	...
	try {
		switch (tab) {
			case "pro": {
				...
				if (!data?.length) {
					el.innerHTML = emptyState("No pro matches");
					return;
				}
				el.innerHTML = `
        ...
        `;
			...
			case "public": {
				...
				if (!data?.length) {
					el.innerHTML = emptyState("No public matches");
					return;
				}
				el.innerHTML = `
          <div class="overflow-table">
            <table class="dota-table">
              ...
              <tbody>
                ${data
					.slice(0, 30)
					.map((m) => {
						const radiantHeroes = (m.radiant_team || "")
							.split(",")
							.filter(Boolean);
						const direHeroes = (m.dire_team || "")
							.split(",")
							.filter(Boolean);
						return `
                    <tr class="cursor-pointer" onclick="window.location.hash='#/matches/${m.match_id}'">
                      ...
                      <td><div class="flex gap-0.5">${radiantHeroes
							.slice(0, 5)
							.map((h) => miniHeroImg(parseInt(h), 20))
							.join("")}</div></td>
                      <td><div class="flex gap-0.5">${direHeroes
							.slice(0, 5)
							.map((h) => miniHeroImg(parseInt(h), 20))
							.join("")}</div></td>
        ...
        `;
			...
			case "parsed": {
				...
				if (!data?.length) {
					el.innerHTML = emptyState("No parsed matches");
					return;
				}
				el.innerHTML = `
				...
				document
					...
					.addEventListener("click", async () => {
						const teamA = document
							.getElementById("find-team-a")
							.value.split(",")
							.filter(Boolean)
							.map(Number);
						const teamB = document
							.getElementById("find-team-b")
							.value.split(",")
							.filter(Boolean)
							.map(Number);
						const resultsEl =
                  ...
                  <tbody>
                    ${data
                      ...
                      <tr class="cursor-pointer" onclick="window.location.hash='#/matches/${m.match_id}'">
                        <td class="text-xs text-dota-gold">${m.match_id}</td>
                        <td class="text-xs">${formatDuration(m.duration)}</td>
                        <td><div class="flex gap-0.5">${(m.radiant_team || "")
							.split(",")
							.filter(Boolean)
							.map((h) => miniHeroImg(parseInt(h), 20))
							.join("")}</div></td>
                        <td><div class="flex gap-0.5">${(m.dire_team || "")
							.split(",")
							.filter(Boolean)
							.map((h) => miniHeroImg(parseInt(h), 20))
							.join("")}</div></td>
