...
export async function renderScenarios() {
  ...
  window.addEventListener('tab-change', (e) => {
    if (e.detail.container === 'scenarios-tabs') {
      document.querySelectorAll('#scenarios-tabs .tab-btn').forEach(b => {
  ...
  try {
    switch (tab) {
      case 'itemtimings': {
        ...
        loadItemTimings();
        document.getElementById('item-filter').addEventListener('input', () => loadItemTimings());
        document.getElementById('hero-filter-items').addEventListener('change', () => loadItemTimings());
        break;
      }
      ...
      case 'laneroles': {
        ...
        loadLaneRoles();
        document.getElementById('lane-role-filter').addEventListener('change', () => loadLaneRoles());
        document.getElementById('hero-filter-lane').addEventListener('change', () => loadLaneRoles());
        break;
      }

      case 'misc': {
        const data = await api.getMiscScenarios();
        if (!data?.length) { el.innerHTML = emptyState('No scenario data'); return; }
...
async function loadItemTimings() {
  ...
  try {
    ...
    if (!data?.length) { el.innerHTML = emptyState('No item timing data'); return; }
...
async function loadLaneRoles() {
  ...
  try {
    ...
    if (!data?.length) { el.innerHTML = emptyState('No lane role data'); return; }
