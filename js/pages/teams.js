...
export async function renderTeams() {
  ...
  `;
	...
	try {
		...
		document.getElementById("teams-list").innerHTML = errorState(
...
function renderTeamsList(teams) {
	...
	if (!teams.length) {
		el.innerHTML = emptyState("No teams found");
		return;
	}
    ...
    `;
		...
		window.addEventListener("tab-change", (e) => {
			if (e.detail.container === "team-tabs") {
				document
					.querySelectorAll("#team-tabs .tab-btn")
					.forEach((b) => {
						b.classList.toggle(
							"active",
							b.textContent.trim().toLowerCase() === e.detail.tab,
						);
					});
	...
	try {
		switch (tab) {
			case "matches": {
				...
				if (!data?.length) {
					el.innerHTML = emptyState("No matches");
					return;
				}
				el.innerHTML = `
        ...
        `;
			...
			case "players": {
				...
				if (!data?.length) {
					el.innerHTML = emptyState("No players");
					return;
				}
				el.innerHTML = `
          <div class="overflow-table">
