...
async function loadProTab(tab) {
	...
	try {
		if (tab === "proplayers") {
			...
			if (!data?.length) {
				el.innerHTML = emptyState("No pro players");
				return;
			}
      ...
      `;
			...
			document
				...
				.addEventListener("input", (e) => {
					...
					document.getElementById("pro-players-list").innerHTML =
						renderProPlayersList(filtered.slice(0, 60));
				});
		} else if (tab === "leagues") {
			const data = await api.getLeagues();
			if (!data?.length) {
				el.innerHTML = emptyState("No leagues");
				return;
			}
			...
			document
				...
				.addEventListener("input", (e) => {
					...
					document.getElementById("leagues-list").innerHTML =
						renderLeaguesList(filtered.slice(0, 50));
				});
		}
	} catch {
    ...
    `;
		...
		window.addEventListener("tab-change", (e) => {
			if (e.detail.container === "league-tabs") {
				document
					.querySelectorAll("#league-tabs .tab-btn")
					.forEach((b) => {
						b.classList.toggle(
							"active",
							b.textContent
	...
	try {
        ...
        `;
			...
			case "teams": {
				...
				if (!data?.length) {
					el.innerHTML = emptyState("No teams");
					return;
				}
				el.innerHTML = `
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        `;
			...
			case "matchids": {
				...
				if (!data?.length) {
					el.innerHTML = emptyState("No match IDs");
					return;
				}
				el.innerHTML = `
