...
export async function renderPlayerProfile(accountId) {
    ...
    `;
		...
		// Refresh button
		document
			.getElementById("refresh-player-btn")
	...
	try {
		switch (tab) {
			case "recent": {
				...
				if (!data?.length) {
					el.innerHTML = emptyState("No recent matches");
					return;
				}
				el.innerHTML = `
        ...
        `;
			...
			case "heroes": {
				...
				if (!data?.length) {
					el.innerHTML = emptyState("No hero data");
					return;
				}
				el.innerHTML = `
          <div class="overflow-table">
        ...
        `;
			...
			case "peers": {
				...
				if (!data?.length) {
					el.innerHTML = emptyState("No peers data");
					return;
				}
				el.innerHTML = `
          <div class="overflow-table">
        ...
        `;
			...
			case "pros": {
				...
				if (!data?.length) {
					el.innerHTML = emptyState("No pro players encountered");
					return;
				}
				el.innerHTML = `
          <div class="overflow-table">
            <table class="dota-table">
              ...
              <tbody>
                ${data
					.filter(
						(p) => (p.with_games || 0) + (p.against_games || 0) > 0,
					)
					.slice(0, 30)
					.map(
						(p) => `
        ...
        `;
			...
			case "totals": {
				...
				if (!data?.length) {
					el.innerHTML = emptyState("No totals data");
					return;
				}
				el.innerHTML = `
            ...
            ${data
				...
				.map((t) => {
					...
					return statCard(
						t.field.replace(/_/g, " "),
						avg,
						`Total: ${formatNumber(t.sum)} | ${t.n} games`,
					);
				})
				.join("")}
          </div>
        `;
			...
			case "counts": {
				...
				if (!data) {
					el.innerHTML = emptyState("No counts data");
					return;
				}
				el.innerHTML = `
        ...
        `;
				...
				document
					.getElementById("histogram-field")
			...
			case "wardmap": {
				...
				if (!data) {
					el.innerHTML = emptyState("No ward data");
					return;
				}
				const obs = data.obs || {};
				const sen = data.sen || {};
        ...
        `;
			...
			case "wordcloud": {
				...
				if (!data) {
					el.innerHTML = emptyState("No word cloud data");
					return;
				}
				const myWords = data.my_word_counts || {};
				const allWords = data.all_word_counts || {};
				const topMy = Object.entries(myWords)
					.sort((a, b) => b[1] - a[1])
					.slice(0, 80);
				const topAll = Object.entries(allWords)
					.sort((a, b) => b[1] - a[1])
					.slice(0, 80);
        ...
        `;
			...
			case "ratings": {
				...
				if (!data?.length) {
					el.innerHTML = emptyState("No rating history");
					return;
				}
				const maxMMR = Math.max(
					...data.map(
						(r) =>
							r.competitive_rank || r.solo_competitive_rank || 0,
					),
				);
				el.innerHTML = `
          <div class="dota-card-flat p-4">
        ...
        `;
			...
			case "rankings": {
				...
				if (!data?.length) {
					el.innerHTML = emptyState("No hero rankings");
					return;
				}
				el.innerHTML = `
          <div class="overflow-table">
        ...
        `;
			...
			case "matches": {
				const data = await api.getPlayerMatches(accountId, {
					limit: 50,
				});
				if (!data?.length) {
					el.innerHTML = emptyState("No matches found");
					return;
				}
				el.innerHTML = `
          <div class="overflow-table">
...
async function loadHistogram(accountId, field) {
	...
	try {
		...
		if (!data?.length) {
			el.innerHTML = emptyState("No histogram data");
			return;
		}
      ...
      <div class="dota-card-flat p-4">
        ...
        <div class="flex items-end gap-[2px] h-48 mb-2">
          ${data
				.map((d) => {
					const height =
						maxGames > 0 ? ((d.games || 0) / maxGames) * 100 : 0;
