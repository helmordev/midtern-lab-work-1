...
import { loader, errorState, emptyState, heroCard, miniHeroImg, statCard, tabBar } from '../components.js';
import { heroImg, heroStore, formatDuration, formatDurationBinLabel, timeAgo, formatNumber, winRate, getAttrClass, getAttrBgClass, getAttrName } from '../utils.js';
...
export async function renderHeroes() {
  ...
  try {
    ...
    document.getElementById('hero-search').addEventListener('input', (e) => {
      const activeAttr = document.querySelector('#attr-filters .filter-chip.active')?.dataset.attr || 'all';
      renderHeroGrid(heroStats, activeAttr, e.target.value);
    });

    // Attribute filter
    document.getElementById('attr-filters').addEventListener('click', (e) => {
      const chip = e.target.closest('.filter-chip');
      if (!chip) return;
      document.querySelectorAll('#attr-filters .filter-chip').forEach(c => {
    ...
    document.getElementById('heroes-grid').innerHTML = errorState('Failed to load heroes');
  }
}
...
function renderHeroGrid(heroes, attr, search) {
  ...
  if (search) {
    ...
    filtered = filtered.filter(h => h.localized_name.toLowerCase().includes(q));
  }
  ...
  grid.innerHTML = `
    <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 gap-2">
      ${filtered.map(hero => {
        ...
        const wrColor = parseFloat(wr) >= 50 ? 'text-dota-radiant' : 'text-dota-dire';
  ...
  try {
    ...
    window.addEventListener('tab-change', function handler(e) {
      if (e.detail.container === 'hero-tabs') {
        document.querySelectorAll('#hero-tabs .tab-btn').forEach(b => {
          b.classList.remove('active');
        });
        e.target.querySelector?.(`.tab-btn[onclick*="${e.detail.tab}"]`)?.classList.add('active');
        // Update active state
        document.querySelectorAll('#hero-tabs .tab-btn').forEach(b => {
          if (b.textContent.trim().toLowerCase().replace(/\s+/g, '') === e.detail.tab.toLowerCase().replace(/\s+/g, '')) {
  ...
  try {
    switch (tab) {
      case 'matchups': {
        ...
        if (!data?.length) { el.innerHTML = emptyState('No matchup data'); return; }
        const sorted = data.sort((a, b) => (b.games_played || 0) - (a.games_played || 0)).slice(0, 30);
        el.innerHTML = `
      ...
      case 'durations': {
        ...
        if (!data?.length) { el.innerHTML = emptyState('No duration data'); return; }
        const chartData = data
          .filter(d => (d.games_played || 0) > 0)
          .sort((a, b) => Number(a.duration_bin) - Number(b.duration_bin));

        if (!chartData.length) { el.innerHTML = emptyState('No duration data'); return; }

        const labels = chartData.map(d => formatDurationBinLabel(d.duration_bin));
        const winRates = chartData.map(d => {
          const games = d.games_played || 0;
          return games > 0 ? Number(((d.wins / games) * 100).toFixed(1)) : 0;
        });
      ...
      case 'players': {
        ...
        if (!data?.length) { el.innerHTML = emptyState('No player data'); return; }
        el.innerHTML = `
          <div class="overflow-table">
            <table class="dota-table">
      ...
      case 'items': {
        ...
        if (!data) { el.innerHTML = emptyState('No item data'); return; }
        const phases = ['start_game_items', 'early_game_items', 'mid_game_items', 'late_game_items'];
        const phaseNames = ['Starting', 'Early Game', 'Mid Game', 'Late Game'];
        el.innerHTML = `
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ${phases.map((phase, i) => {
              const items = data[phase] || {};
              const sorted = Object.entries(items).sort((a, b) => b[1] - a[1]).slice(0, 8);
                ...
                <div class="dota-card-flat p-4">
                  ...
                  <div class="space-y-2">
                    ${sorted.map(([itemId, count]) => {
                      const itemName = heroStore.getItemName(itemId) || itemId;
      ...
      case 'rankings': {
        ...
        if (!data?.rankings?.length) { el.innerHTML = emptyState('No ranking data'); return; }
        el.innerHTML = `
          <div class="overflow-table">
            <table class="dota-table">
              <thead><tr><th>#</th><th>Player</th><th>Score</th><th>Account</th></tr></thead>
              <tbody>
                ${data.rankings.slice(0, 30).map((r, i) => `
      ...
      case 'benchmarks': {
        ...
        if (!data?.result) { el.innerHTML = emptyState('No benchmark data'); return; }
        const benchmarks = Object.entries(data.result);
        el.innerHTML = `
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ${benchmarks.map(([key, values]) => {
      ...
      case 'matches': {
        ...
        if (!data?.length) { el.innerHTML = emptyState('No match data'); return; }
        el.innerHTML = `
          <div class="overflow-table">
            <table class="dota-table">
